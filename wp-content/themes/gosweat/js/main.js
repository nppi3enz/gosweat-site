...
function loadMusic(id){
  ...
  if(data_music[id].link_joox !== null) output_stream += '<a target="_blank" href="'+data_music[id].link_joox+'" class="streaming-btn"><img src="/wp-content/uploads/2018/07/logo-joox.png"></a>';
  jQuery('.streaming-music-area').html(output_stream);
  jQuery("#video")[0].src += "&autoplay=1";
}
jQuery(document).on('click', '.music-list', function() {
  var id_song = jQuery(this).data('song');
  jQuery('.music-list').removeClass('select');
  jQuery(this).addClass('select');
  loadMusic(id_song);
});
function callMember(id){
  jQuery('#member-info .member-left').html('<img src="'+data_member[id].pic+'">');
  jQuery('#member-info .name-title').text(data_member[id].title);
  jQuery('#member-info .description').html(data_member[id].detail);
  jQuery('#member-info .social-area .facebook').attr('href', data_member[id].facebook);
  jQuery('#member-info .social-area .twitter').attr('href', data_member[id].twitter);
  jQuery('#member-info .social-area .instagram').attr('href', data_member[id].instagram);
  jQuery('#member-info .social-area .twitch').attr('href', data_member[id].twitch);
  jQuery('#member-info').fadeIn();
  jQuery('#member-info .member-left').animateCss('fadeInLeft');
  jQuery('#member-info .member-right').animateCss('fadeInRight');
}
jQuery(document).on('click', '.member-btn', function() {
